...
type (
	stockModel interface {
		...
		DecuctStock(ctx context.Context, goodsId, num int64) (sql.Result, error)
		AddStock(ctx context.Context, goodsId, num int64) error
	}
...
func (m *defaultStockModel) withSession(session sqlx.Session) *defaultStockModel {
	return &defaultStockModel{
...
func (m *defaultStockModel) DecuctStock(ctx context.Context, goodsId, num int64) (sql.Result, error) {
	query := fmt.Sprintf("update %s set `num` = `num` - ? where `goods_id` = ? and num >= ?", m.table)
	return m.conn.ExecCtx(ctx, query, num, goodsId, num)

}

func (m *defaultStockModel) AddStock(ctx context.Context, goodsId, num int64) error {
	query := fmt.Sprintf("update %s set `num` = `num` + ? where `goods_id` = ?", m.table)
	_, err := m.conn.ExecCtx(ctx,query, num, goodsId)
	return err
}
